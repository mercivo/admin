...
describe('AuthService', () => {
  ...
  beforeAll(async () => {
    const user = { id: 'user-1', phone: '13800000000', username: null, passwordHash: await hashPassword('TenantAdmin@2026'), role: 'admin', status: 'active', tenantId: 'tenant-1', siteId: 'site-1', lastLoginAt: null };
  ...
  it('returns a tenant-scoped token for valid credentials', async () => {
    const result = await service.login({ account: '13800000000', password: 'TenantAdmin@2026' });
